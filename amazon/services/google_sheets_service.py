...
def _build_google_api_session() -> requests.Session:
    session = requests.Session()
    adapter = HTTPAdapter(pool_connections=10, pool_maxsize=20, max_retries=_GOOGLE_API_RETRY)
    session.mount("https://", adapter)
    session.mount("http://", adapter)
    # requestsは既定でWindowsのシステムプロキシ設定を自動検出して使う（trust_env=True）。
    # ウイルス対策ソフト等がHTTPS検査のためシステムプロキシ経由の通信を横取りしている場合、
    # ブラウザ向けには正常に動いていても、別アプリ（このPythonプロセス）からの通信だけ詰まる
    # ことがある。ここでは無視して直接接続させる（プロキシが必要な環境なら別途要検討）。
...
# で保持していたプール中のTCP接続を、その間にルーター側のNATがこっそり切っていて、次に使った
# ときだけ「接続はできるが応答が返らない」状態になっていたと考えられる。一定時間使われていない
# セッションは使う前に安全側で作り直す（操作内の連続コールは数秒以内なので、張り直しの影響は
# 受けず今まで通り使い回される）。
_GOOGLE_API_SESSION_IDLE_LIMIT = 60  # 秒
_google_api_session_last_used = 0.0


def _google_session() -> requests.Session:
    global _GOOGLE_API_SESSION, _google_api_session_last_used
    now = time.monotonic()
    if now - _google_api_session_last_used > _GOOGLE_API_SESSION_IDLE_LIMIT:
...
def get_receipt_settings(user_id: int) -> dict:
    # 列がまだ無い等でクエリが失敗しても、接続をプールへ返し（finally）、
    # 画面が壊れないよう空設定を返す（取込実行時に「フォルダ未設定」で弾かれる）。
    conn = get_conn("a_orbit_dispatch_sheet_settings.db")
    try:
        cur = conn.cursor()
        cur.execute(
        ...
        import traceback
        print("[get_receipt_settings] ERROR")
        traceback.print_exc()
        return {"inbox_dir": "", "store_dir": ""}
    finally:
        conn.close()
...
def fetch_kanrihin_sheet_rows(user_id: int) -> dict:
    ...
    タブ名・スプレッドシートURLは同じ設定行の別列なので1クエリでまとめて取得する。"""
    conn = get_conn("a_orbit_dispatch_sheet_settings.db")
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT spreadsheet_url, kanrihin_sheet_name FROM orbit_dispatch_sheet_settings WHERE user_id = %s",
            (user_id,),
        )
        row = cur.fetchone()
    finally:
    ...
    sheet_name = row["kanrihin_sheet_name"] if row and row.get("kanrihin_sheet_name") else ""
    if not sheet_name:
        raise RuntimeError("管理品タブ名が未設定です（管理品タブの設定欄で指定してください）")

    spreadsheet_url = row["spreadsheet_url"] if row and row.get("spreadsheet_url") else ""
